...
import { Search, Plus, Upload, Download, LayoutGrid, List, X, AlertCircle, SlidersHorizontal } from 'lucide-react'
...
export function InfluencersClient({ initialInfluencers, profiles }: InfluencersClientProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return list
  }, [influencers, search, stageFilter, categoryFilter, tagFilter, overdueOnly, today, minFollowers, maxFollowers])

  const followersRange = `${minFollowers ? minFollowers + '+' : ''}${minFollowers && maxFollowers ? '~' : ''}${maxFollowers ? maxFollowers + '-' : ''}`.trim()

  const activeFilters = [
    stageFilter && { key: 'stage', label: tStage(stageFilter as InfluencerStage) },
    categoryFilter && { key: 'category', label: tCategory(categoryFilter) },
    tagFilter && { key: 'tag', label: `#${tagFilter}` },
    overdueOnly && { key: 'overdue', label: t('influencers.overdueLabel') },
    (minFollowers || maxFollowers) && {
      key: 'followers',
      label: t('influencers.followersBadge', { range: followersRange }),
    },
  ...
  function handleExport() {
    ...
    // Header row uses the active locale's CSV header dictionary.
    const displayHeaders = headers.map((h) => t(`influencers.csvHeaders.${h}`))
    ...
    const rows = filtered.map((i) => {
      ...
      return headers.map((h) => {
        ...
        if (h === 'tags') val = (i.tags ?? []).join(';')
        if (typeof val === 'boolean') val = val ? yesLabel : noLabel
  ...
  // Tighten dependency on locale so memoized derivations refresh after a swap.
  void locale
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 md:px-6 py-3 md:py-4 border-b bg-white">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <h1 className="text-lg font-semibold text-gray-900 mr-auto">{t('influencers.title')}</h1>
          ...
          >
            ...
            {t('influencers.filters')}{activeFilters.length > 0 ? ` (${activeFilters.length})` : ''}
          </button>
          ...
          <div className="flex items-center border rounded-md overflow-hidden">
            ...
            >
              <List className="h-4 w-4" /><span className="hidden sm:inline">{t('influencers.table')}</span>
            </button>
            <button
            ...
            >
              <LayoutGrid className="h-4 w-4" /><span className="hidden sm:inline">{t('influencers.kanban')}</span>
            </button>
          </div>

          <Button variant="outline" size="sm" onClick={() => setShowImport(true)} className="hidden sm:flex">
            <Upload className="h-4 w-4 mr-1.5" />{t('influencers.import')}
          </Button>
          <Button variant="outline" size="sm" onClick={handleExport} className="hidden sm:flex">
            <Download className="h-4 w-4 mr-1.5" />{t('influencers.export')}
          </Button>
          <Button size="sm" onClick={() => setShowAdd(true)}>
            <Plus className="h-4 w-4 mr-1.5" /><span className="hidden sm:inline">{t('influencers.addInfluencer')}</span><span className="sm:hidden">{t('influencers.addShort')}</span>
          </Button>
        </div>
        ...
        <div className={`mt-2 flex items-center gap-2 flex-wrap ${showFilters ? 'flex' : 'hidden md:flex'}`}>
          ...
          >
            <option value="">{t('influencers.allStages')}</option>
            {ALL_STAGES.map((s) => <option key={s} value={s}>{tStage(s)}</option>)}
          </select>
          ...
          {categories.length > 0 && (
            ...
            >
              <option value="">{t('influencers.allCategories')}</option>
          ...
          {allTags.length > 0 && (
            ...
            >
              <option value="">{t('influencers.allTags')}</option>
          ...
          <div className="flex items-center gap-1">
            <input
              type="number"
              min={0}
              placeholder={t('influencers.minFollowers')}
              value={minFollowers}
            ...
            <input
              type="number"
              min={0}
              placeholder={t('influencers.maxFollowers')}
              value={maxFollowers}
      ...
      <div className="px-6 py-2 text-sm text-gray-500 bg-white border-b flex items-center gap-2 flex-wrap min-h-[38px]">
        <span>
          {(activeFilters.length > 0 || search)
            ? t('influencers.foundCount', { found: filtered.length, total: influencers.length })
            : t('influencers.totalCount', { total: influencers.length })}
        </span>
        {activeFilters.map((f) => (
